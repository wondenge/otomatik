...
// An empty Config is not valid: use New() to obtain a valid Config.
type Config struct {
	// How much of a certificate's lifetime becomes the renewal window,
	// which is the span of time at the end of the certificate's validity period in which it should be renewed;
	// for most certificates, the global default is good, but for extremely short-lived certs, you may want to raise this to ~0.5.
	RenewalWindowRatio float64

	// An optional event callback clients can set to subscribe to certain things happening
	// internally by this config; invocations are synchronous, so make them return quickly!
	OnEvent func(event string, data interface{})

	// DefaultServerName specifies a server name to use when choosing a
	// certificate if the ClientHello's ServerName field is empty
	DefaultServerName string

	// The state needed to operate on-demand TLS;
	// if non-nil, on-demand TLS is enabled and certificate operations are deferred to TLS handshakes (or as-needed)
	...
	// The type that issues certificates;
	// the default Issuer is ACMEManager
	Issuer Issuer

	// The type that revokes certificates;
	// must be configured in conjunction with the Issuer
	// field such that both the Issuer and Revoker are related (because issuance information is required for revocation)
	Revoker Revoker
	...
	// CertSelection chooses one of the certificates with which the ClientHello will be completed;
	// if not set, DefaultCertificateSelector will be used
	CertSelection CertificateSelector

	// The storage to access when storing or loading TLS assets
	Storage Storage
...
// Most users will call this function instead of New() since most use cases require only a single config for any and all certificates.
// If your requirements are more advanced (for example, multiple configs depending on the certificate), then use New() instead.
// (You will need to make your own Cache first.)
// If you only need a single Config to manage your certs (even if that config changes, as long as it is the only one),
// customize the Default package variable before calling NewDefault().
// All calls to NewDefault() will return configs that use the same, default certificate cache.
// All configs returned by NewDefault() are based on the values of the fields of Default at the time it is called.
func NewDefault() *Config {
	defaultCacheMu.Lock()
	if defaultCache == nil {
		defaultCache = NewCache(CacheOptions{
			// the cache will likely need to renew certificates, so it will need to know how to do that,
			// which depends on the certificate being managed and which can change during the lifetime of the cache;
			// this callback makes it possible to get the latest and correct config with which to manage the cert,
			// but if the user does not provide one, we can only assume that we are to use the default config
...
// ManageSync causes the certificates for domainNames to be managed according to cfg.
// If cfg.OnDemand is not nil, then this simply whitelists the domain names and defers the certificate
// operations to when they are needed.
// Otherwise, the certificates for each name are loaded from storage or obtained from the CA.
// If loaded from storage, they are renewed if they are expiring or expired.
// It then caches the certificate in memory and is prepared to serve them up during TLS handshakes.
//
// Note that name whitelisting for on-demand management only takes effect if cfg.OnDemand.DecisionFunc is not set (is nil);
// it will not overwrite an existing DecisionFunc, nor will it overwrite its decision;
// i.e. the implicit whitelist is only used if no
// DecisionFunc is set.
//
// This method is synchronous, meaning that certificates for all domainNames must be
// successfully obtained (or renewed) before it returns.
// It returns immediately on the first error for any of the given domainNames.
// This behavior is recommended for interactive use
// (i.e. when an administrator is present) so that errors can be reported and fixed immediately.
func (cfg *Config) ManageSync(domainNames []string) error {
	return cfg.manageAll(nil, domainNames, false)
}

// ManageAsync is the same as ManageSync, except that ACME operations are performed asynchronously (in the background).
// This method returns before certificates are ready.
// It is crucial that the administrator monitors the logs and is notified of any errors
// so that corrective action can be taken as soon as possible.
// Any errors returned from this method occurred before ACME transactions started.
//
// As long as logs are monitored, this method is typically
// recommended for non-interactive environments.
//
// If there are failures loading, obtaining, or renewing a certificate, it will be retried with exponential backoff
// for up to about 30 days, with a maximum interval of about 24 hours.
// Cancelling ctx will cancel retries and shut down any goroutines spawned by ManageAsync.
...
func (cfg *Config) manageOne(ctx context.Context, domainName string, async bool) error {
	...
	if err != nil {
		...
		if async {
			// Leave the job name empty so as to allow duplicate 'obtain' jobs;
			// this is because Caddy calls ManageAsync() before the previous config is stopped
			// (and before its context is canceled), which means that if an obtain job is still running for the same domain,
			// Submit() would not queue the new one because it is still running, even though it is (probably)
			// about to be canceled (it might not if the new config fails to finish loading, however).
			// In any case, we presume it is safe to enqueue a duplicate obtain job because either the
			// old one (or sometimes the new one) is about to be canceled.
			// This seems like reasonable logic for any consumer of this lib.
...
// ObtainCert obtains a certificate for name using cfg, as long as a certificate does not already exist in storage for that name.
// The name must qualify and cfg must be flagged as Managed.
// This function is a no-op if storage already has a certificate for name.
// It only obtains and stores certificates (and their keys), it does not load them into memory.
// If interactive is true, the user may be shown a prompt.
// TODO: consider moving interactive param into the Config struct, and maybe retry settings into the Config struct as well? (same for RenewCert)
...
// RenewCert renews the certificate for name using cfg.
// It stows the renewed certificate and its assets in storage if successful.
// It DOES NOT update the in-memory cache with the new certificate.
...
// It requires that cfg.Issuer is properly configured with the same issuer that issued the certificate being revoked.
...
// TLSConfig is an opinionated method that returns a recommended, modern TLS configuration that can be used
// to configure TLS listeners, which also supports the TLS-ALPN challenge and serves up certificates managed by cfg.
// Unlike the package TLS() function, this method does not, by itself, enable certificate management for any domain names.
// Feel free to further customize the returned tls.Config, but do not mess with the GetCertificate
// or NextProtos fields unless you know what you're doing, as they're necessary to solve the TLS-ALPN challenge.
...
// getPrecheckedIssuer returns an Issuer with pre-checks completed, if it is also a PreChecker.
// It also checks that storage is functioning.
// If a nil Issuer is returned with a nil error, that means to skip this operation (not an error, just a no-op).
func (cfg *Config) getPrecheckedIssuer(names []string, interactive bool) (Issuer, error) {
	...
	// perform check once every so often for each storage, which may require some global state...
...
// checkStorage tests the storage by writing random bytes to a random key,
// and then loading those bytes and comparing the loaded value.
// If this fails, the provided cfg.Storage mechanism should not be used.
func (cfg *Config) checkStorage() error {
	...
	defer func() {
		...
		// if there was no other error, make sure to return any error returned from Delete
		if err == nil {
			err = deleteErr
		}
	}()
...
// storageHasCertResources returns true if the storage associated with cfg's
// certificate cache has all the resources related to the certificate for domain:
// the certificate, the private key, and the metadata.
...
// lockKey returns a key for a lock that is specific to the operation named op
// being performed related to domainName and this config's CA.
...
// or if the process of checking the expiration returned an error.
